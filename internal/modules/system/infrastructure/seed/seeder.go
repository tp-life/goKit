...
func (s *Seeder) Run(ctx context.Context) error {
	...
	// 1. 根部门
	rootDept := &entity.Dept{ParentID: 0, Name: "总公司", Sort: 0, Status: 1}
...
func (s *Seeder) seedMenus(ctx context.Context) ([]uint64, error) {
	...
	create := func(parentID uint64, title string, typ int8, path, perm string, sort int) (uint64, error) {
		m := &entity.Menu{ParentID: parentID, Title: title, Type: typ, Path: path, PermCode: perm, Sort: sort, Status: 1}
	...
	sysDir, err := create(0, "系统管理", entity.MenuTypeDir, "/system", "", 1)
	if err != nil {
		return nil, err
	}
	...
	for _, res := range resources {
		menuID, err := create(sysDir, res.title, entity.MenuTypeMenu, res.path, res.code+":list", res.sort)
		if err != nil {
			return nil, err
		}
		for i, act := range append(actions[1:], res.extra...) {
			if _, err := create(menuID, res.title+"-"+act, entity.MenuTypeButton, "", res.code+":"+act, i+1); err != nil {
				return nil, err
			}
		}
	}
