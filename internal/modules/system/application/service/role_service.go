...
func toRoleResp(r *entity.Role) *dto.RoleResp {
	return &dto.RoleResp{
		ID:        r.ID,
		Name:      r.Name,
		Code:      r.Code,
...
func (s *RoleService) Create(ctx context.Context, req dto.CreateRoleReq) (uint64, error) {
	role := &entity.Role{
		Name:      req.Name,
		Code:      req.Code,
...
func (s *RoleService) Update(ctx context.Context, id uint64, req dto.UpdateRoleReq) error {
...
func (s *RoleService) List(ctx context.Context, page dto.PageReq) (*dto.PageResp[*dto.RoleResp], error) {
	page.Normalize()
	roles, total, err := s.roleRepo.List(ctx, page.Page, page.PageSize)
	if err != nil {
		return nil, err
	}
	list := make([]*dto.RoleResp, 0, len(roles))
	for i := range roles {
		list = append(list, toRoleResp(&roles[i]))
	}
	return &dto.PageResp[*dto.RoleResp]{List: list, Total: total}, nil
}

// AssignMenus 给角色分配菜单权限（全量重置）
func (s *RoleService) AssignMenus(ctx context.Context, id uint64, req dto.AssignMenusReq) error {
	if _, err := s.mustGet(ctx, id); err != nil {
		return err
	}
...
func (s *RoleService) AssignUsers(ctx context.Context, id uint64, req dto.AssignUsersReq) error {
	if _, err := s.mustGet(ctx, id); err != nil {
		return err
	}
...
func (s *RoleService) mustGet(ctx context.Context, id uint64) (*entity.Role, error) {
