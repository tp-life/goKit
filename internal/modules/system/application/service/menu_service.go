...
func (s *MenuService) Create(ctx context.Context, req dto.CreateMenuReq) (uint64, error) {
	menu := &entity.Menu{
...
func (s *MenuService) Update(ctx context.Context, id uint64, req dto.UpdateMenuReq) error {
...
func (s *MenuService) Tree(ctx context.Context) ([]*dto.MenuNode, error) {
	...
	nodes := make(map[uint64]*dto.MenuNode, len(menus))
	for _, m := range menus {
		nodes[m.ID] = &dto.MenuNode{
			ID:       m.ID,
			ParentID: m.ParentID,
			Title:    m.Title,
			Type:     m.Type,
