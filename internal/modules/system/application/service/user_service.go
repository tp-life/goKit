...
func (s *UserService) Create(ctx context.Context, req dto.CreateUserReq) (uint64, error) {
...
func (s *UserService) Update(ctx context.Context, id uint64, req dto.UpdateUserReq) error {
...
func (s *UserService) List(ctx context.Context, page dto.PageReq) (*dto.PageResp[*dto.UserResp], error) {
	page.Normalize()
	...
	return &dto.PageResp[*dto.UserResp]{List: list, Total: total}, nil
}

// AssignRoles 给用户分配角色（全量重置）
func (s *UserService) AssignRoles(ctx context.Context, id uint64, req dto.AssignRolesReq) error {
	if _, err := s.mustGet(ctx, id); err != nil {
		return err
	}
...
func (s *UserService) ResetPassword(ctx context.Context, id uint64, req dto.ResetPwdReq) error {
	if _, err := s.mustGet(ctx, id); err != nil {
		return err
	}
	hashed, err := bcrypt.GenerateFromPassword([]byte(req.Password), bcrypt.DefaultCost)
...
func (s *UserService) mustGet(ctx context.Context, id uint64) (*entity.User, error) {
