// Package datascope 数据权限领域服务：不依赖任何框架的纯业务逻辑
package datascope

import "goKit/internal/domain/role"
...
func MergeDataScope(userID uint64, isSuper bool, roles []RoleDataScope) Filter {
	...
	for _, r := range roles {
		switch r.Scope {
		case role.DataScopeAll:
			return Filter{All: true}
		case role.DataScopeCustom, role.DataScopeDeptAndChildren, role.DataScopeDept:
			for _, id := range r.DeptIDs {
				deptSet[id] = struct{}{}
			}
		case role.DataScopeSelf:
			self = true
		}
	}
