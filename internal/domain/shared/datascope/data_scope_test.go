...
func TestMergeDataScope(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:    "任一角色为全部则全部",
			userID:  1,
			roles:   []RoleDataScope{{Scope: role.DataScopeSelf}, {Scope: role.DataScopeAll}},
			wantAll: true,
		},
		{
			name:   "自定义与本部门合并取并集",
			userID: 1,
			roles: []RoleDataScope{
				{Scope: role.DataScopeCustom, DeptIDs: []uint64{10, 11}},
				{Scope: role.DataScopeDept, DeptIDs: []uint64{11, 12}},
			},
		...
		},
		{
			name:     "仅本人",
			userID:   7,
			roles:    []RoleDataScope{{Scope: role.DataScopeSelf}},
			wantSelf: 7,
		},
		{
			name:   "部门范围叠加仅本人",
			userID: 7,
			roles: []RoleDataScope{
				{Scope: role.DataScopeDeptAndChildren, DeptIDs: []uint64{20}},
				{Scope: role.DataScopeSelf},
			},
			wantDept: []uint64{20},
			wantSelf: 7,
		},
		...
		},
		{
			name:     "自定义范围为空且无本人则拒绝",
			userID:   1,
			roles:    []RoleDataScope{{Scope: role.DataScopeCustom}},
			wantDeny: true,
		},
	}
